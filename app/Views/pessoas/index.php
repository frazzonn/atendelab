    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody id="tabelaPessoas">
                <tr>
                    <td colspan="8" class="text-center py-4">Carregando...</td>
                </tr>
            </tbody>
        </table>
    </div>
...
<script>
    ...
    async function carregarPessoas() {
        try {
            ...
            if (!dados.length) { tbody.innerHTML = '<tr><td colspan="8" class="text-center py-4">Nenhuma pessoa cadastrada.</td></tr>'; return; }
            tbody.innerHTML = dados.map(p => {
                const emAberto = Number(p.atendimentos_abertos || 0);
                const badgeAberto = emAberto > 0
                    ? `<span class="badge text-bg-warning">${emAberto}</span>`
                ...
                <td class="text-center">${badgeAberto}</td>
